...
export async function optimizeRoute(addresses: Address[], startAddress?: Address) {
  ...
  while (unvisited.length > 0) {
    ...
    const currentStop = nearest;
    if (currentStop && typeof currentStop.time_spent === 'number') {
      totalDuration += currentStop.time_spent;
    }
  }
...
function optimizeRouteWithAppointments(addresses: Address[], startAddress?: Address) {
  ...
  if (route.length > 0) {
    ...
    if (lastStop && typeof lastStop.time_spent === 'number') {
